...
- **Sexta pasada de perf (2026-08-25, misma sesión) -- implementación real: reemplazo de `DrawOP_COPY_Compress_16`, verificado parcialmente en consola:**
  ...
  - **Estado actual (actualizado 2026-08-27):** el usuario decidió activar `OPT_COPY_BLIT_REWRITE=ON` por defecto en `CMakeLists.txt` (antes apagado) dado que la verificación previa no mostró errores gráficos en caminata/menús/combate normal. Bosses/cinemáticas/otros mapas siguen sin cobertura exhaustiva -- si una sesión futura encuentra corrupción de sprites en ese contenido, es el primer sospechoso a revertir a `OFF`.

- **Séptima pasada de perf (2026-08-27) -- resultado medido de `OPT_COPY_BLIT_REWRITE=ON` + affinity de CPU combinados, y primera lectura de uso por-core:**
  - **Mejora real confirmada en consola:** con `OPT_COPY_BLIT_REWRITE=ON` (ya default) más la afinidad de CPU del main thread a core 0 (commit `a306e8d`), el piso de FPS en combate subió de los ~7 FPS reportados en la 4ª pasada a **12 FPS mínimo**. Primera mejora medida del suelo de framerate, no solo del promedio.
  - **Re-análisis de `advena_041.log` (mismo log de la 6ª pasada, releído completo):** 580 líneas `[PERF] GL/frame`, ninguna con telemetría de CPU/GPU por core (esa instrumentación no existe en este proyecto todavía). Pico máximo de esta sesión = **660 blits/frame** (avg 406.4), distribución igual a la ya documentada (`COPY` ~79%). Sigue sin alcanzar el pico de 1975 de la sesión original con 10+ enemigos -- la pregunta abierta de la 6ª pasada (bajón al golpear: ¿mismo cuello de botella de render o lógica de juego?) sigue sin resolverse con este log.
  - **Nuevo dato -- overlay de FPS/CPU/GPU (plugin tipo PSVshell activado desde VitaShell, no instrumentación propia del proyecto):** el usuario reporta **core 0 y GPU constantemente al 100%**, con los demás cores de usuario más equilibrados/con margen. Esto es consistente con el diseño actual: el main thread (pineado a core 0 desde la 5ª pasada/`a306e8d`) ejecuta en serie TODO el tick de lógica + la composición por software de sprites (`PutCompressImg`/`DrawOP_*`) en un solo hilo -- es esperable que sature su propio core mientras los otros quedan con holgura. La lectura de GPU al 100% es más ambigua: el pipeline GL real de este juego es 1 solo `glTexSubImage2D` (480x320, ~300KB) + 1 quad de pantalla completa por frame (confirmado en la 4ª pasada), carga de cómputo GPU trivial para el hardware -- un overlay externo puede reportar "GPU 100%" incluyendo tiempo de espera de vsync/sync fence como "ocupado", no necesariamente cómputo real. No se puede confirmar con este dato solo si el GPU es un cuello de botella genuino o un artefacto de medición del overlay.
  - **Conclusión / candidatos para la próxima pasada:** (1) confirmar la lectura de GPU con una medición propia (timestamps alrededor de `glTexSubImage2D`+draw en `glutil.c`, no depender del overlay externo) antes de invertir esfuerzo ahí; (2) dado que core 0 está saturado y los otros cores de usuario tienen margen, evaluar si alguna fase de la composición por software es separable a otro hilo -- sigue siendo trabajo de alto riesgo/esfuerzo (el `.so` es una caja cerrada que compone sprites en un único buffer compartido en un orden dependiente de z-order, no es un problema de paralelización trivial) pero ahora con mejor justificación que antes de tener este dato de holgura en los otros cores; (3) seguir pendiente la reescritura de `BLEND16`/`DARKEN`/`ADD` (16.7% de los blits) con el mismo método que `COPY`, la ganancia de menor riesgo que sigue disponible sin tocar threading ni GPU.

  - **Implementación real (misma sesión, misma pasada) -- `DrawOP_DARKEN_Compress_16` y `DrawOP_ADD_Compress_16` reescritos completos, `DrawOP_BLEND16_Compress_16` con fix parcial:** siguiendo el ítem (3) de arriba, se resolvieron los offsets reales de las tres funciones vía `nm -D` sobre el `.so` real (son símbolos exportados igual que `PutCompressImg`/`DrawOP_COPY_Compress_16`, no hubo que adivinarlos): `DrawOP_DARKEN_Compress_16` = `.so+0x13d888`, `DrawOP_ADD_Compress_16` = `.so+0x13cf0c`, `DrawOP_BLEND16_Compress_16` = `.so+0x13d388` (verificado cruzando que el mismo método reproduce el offset ya conocido de `DrawOP_COPY_Compress_16`, `0x142588`, exacto).
    - **`DrawOP_DARKEN_Compress_16` (reemplazo total, igual patrón que `COPY`):** mínimo por canal (R/G/B) entre destino y paleta, sin parámetro de alpha ni tablas -- byte-exacto contra `out_ghidra.c:256313-256385`. Sin riesgo de datos faltantes.
    - **`DrawOP_ADD_Compress_16` (reemplazo total):** suma saturada por canal ponderada por un `alpha` (0 < alpha < 0x100); `alpha == 0xff` es suma saturada pura, cualquier otro valor pondera la contribución de la fuente por `alpha/256` antes de sumar. Sin tablas -- byte-exacto contra `out_ghidra.c:255717-255849`.
    - **`DrawOP_BLEND16_Compress_16` (fix PARCIAL, no reemplazo total):** el caso `alpha == 8` (punto medio 50/50 de su rango 1-15) usa el mismo truco de máscara-y-shift para promediar sin tablas (`(x & 0xf7de) >> 1`, byte-exacto). Los demás 14 valores de alpha dependen de dos tablas precalculadas en el `.rodata` del `.so` (`DAT_00175d90`/`DAT_00175e80` en la nomenclatura de Ghidra) cuyos VALORES reales no se pudieron recuperar con confianza del ELF -- la dirección referenciada (`0x175d90`) cae en un hueco entre los dos segmentos `PT_LOAD` del `.so` tal como está compilado, así que un volcado directo por offset de archivo no es confiable. Adivinar una fórmula "equivalente" en vez de la tabla real arriesgaba corromper colores de blend en cualquier nivel de alpha mal adivinado -- silencioso, no un crash. Se optó por un hook de tipo guardia (restaurar-llamar-reparchear, mismo patrón que `CSaveMgr_Save_patched` en este archivo) que solo acelera `alpha == 8` y cae al original sin tocar para cualquier otro alpha.
    - **Gateado tras un flag nuevo:** `OPT_MORE_BLIT_REWRITE` (`CMakeLists.txt`).
    - **Verificación en consola real (2026-08-28, build release con `-DOPT_MORE_BLIT_REWRITE=ON`):** el usuario confirmó **sin errores visuales** jugando en una build release. Igual que con `OPT_COPY_BLIT_REWRITE`, se activó por defecto (`ON`) tras esta confirmación -- misma disciplina, no antes.

- **Octava pasada (2026-08-28) -- se descarta paralelizar la composición por software entre cores, se reemplaza por lectura directa de las tablas de `BLEND16`:**
  - **Pedido original:** el usuario reportó (con el overlay de la 7ª pasada) que todo el trabajo de composición cae en el core 0 y pidió repartirlo en "los otros 3 cores".
  - **Corrección de expectativa:** PS Vita solo expone 3 cores de usuario (`SCE_KERNEL_CPU_MASK_USER_0/1/2`, el 4to es del sistema). De esos 3, core 0 ya es el main thread (lógica+render) y core 1 ya es el mixer de audio (`a306e8d`) -- **solo el core 2 está genuinamente libre**. El máximo reparto real posible es entre 2 hilos (main thread + 1 helper en core 2), no 4.
  - **Por qué se descartó el paralelismo por ahora:** los sprites se componen en un único framebuffer compartido en un orden específico (fondo -> personajes -> efectos, cada blit pinta encima del anterior). Repartir los blits de un frame entre 2 hilos por mitad de lista (sin más cuidado) no garantiza que sprites superpuestos en pantalla se sigan dibujando en el mismo orden -- riesgo real de parpadeo/capas mal ordenadas, intermitente y probablemente solo visible en combates densos (el escenario que se quiere arreglar), muy difícil de diferenciar de "sigue lento" en una prueba corta. La alternativa correcta (partición por bandas de filas de pantalla) requeriría conocer la dirección base real del framebuffer de 480x320 del `.so`, que no está confirmada, y no se puede verificar sin varias rondas de prueba visual en consola. Se le presentó esta disyuntiva al usuario, que eligió priorizar la alternativa más segura de abajo en su lugar.
  - **Alternativa implementada -- lectura directa de memoria en vez de GDB:** para completar el path general de `DrawOP_BLEND16_Compress_16` (alpha != 8, pendiente desde la 7ª pasada por depender de dos tablas del `.rodata` cuyos valores no se pudieron leer con confianza desde el ELF estático), en vez de montar un gdbstub completo se agregó un volcado diagnóstico (`INSTRUMENT_BLEND16_TABLES`, off por defecto, `source/patch.c`: `dump_blend16_tables_once()`) que lee esas mismas tablas directamente de la memoria mapeada del proceso en ejecución (`so_mod.text_base + 0x175d90`/`0x175e80` -- la misma base que ya usa cada hook de este archivo) y las escribe a `game_log()`/`l_info()` la primera vez que el juego use un alpha distinto de 8. Es el mismo dato que daría GDB, sin la infraestructura nueva de un gdbstub -- 15 niveles x (8 máscaras `ushort` + 8 shifts `byte`).
  - **Pendiente (requiere consola física):** compilar con `-DINSTRUMENT_BLEND16_TABLES=ON` (además de `-DOPT_MORE_BLIT_REWRITE=ON`), jugar hasta que ocurra un blend con alpha != 8 (efectos de oscurecido/highlight con intensidad distinta al 50%), y bajar el log por FTP. Con esas 15 líneas `[Patch] level=... masks=... shifts=...` se puede escribir la reescritura completa de `DrawOP_BLEND16_Compress_16` sin dejar ningún alpha sin optimizar.
