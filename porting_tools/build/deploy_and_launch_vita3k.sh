...
#
# Requiere que el directorio fuente temporal /tmp/advena-src y el directorio de build
# /tmp/advena-build ya estén configurados (el script build_and_install.sh hace esto).
#   mkdir -p /tmp/advena-build && cd /tmp/advena-build && cmake /tmp/advena-src -DCMAKE_BUILD_TYPE=Debug
#
# Uso: ./porting_tools/build/deploy_and_launch_vita3k.sh
set -e
