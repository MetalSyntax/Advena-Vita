...
# variantes de build (advena.vpk, advena_debug.vpk, etc.) sin
# tener que hardcodear cada nombre.
BUILD_DIR = "build"
VITA_DOWNLOADS_DIR = "/ux0:/downloads"
VITA_DATA_DIR = "/ux0:/data"
VITA_LOGS_DIR = "/ux0:/data/advena/logs"
VITA_CG_DIR = "ux0:/data/advena/cg"
...
def upload_eboot():
    ...
    choice = input(f"¿Deseas subir SOLO el eboot.bin a ux0:/app/{VITA_TITLEID}/? [S/n] (0 / 'q' para volver al menú): ").strip().lower()
    if choice in ['0', 'q', 'n', 'no', 'c', 'cancel', 'r', 'volver']:
        print("[*] Operación cancelada. Regresando al menú principal...")
        return
    ...
    dest_dir = f"/ux0:/app/{VITA_TITLEID}"
...
def download_latest_debug_files():
    ...
    if descargar_dmp in ['s', 'y', 'si', 'yes']:
        ...
        try:
            ...
            if latest_dmp:
                ...
                local_dmp_name = os.path.join(logs_folder, f"Advena-{latest_dmp}")
    ...
    print(f"[*] Buscando el último log de Advena en {VITA_LOGS_DIR}...")
    latest_log = None
    latest_log_time = 0
    try:
...
def download_glsl_shaders():
    ...
    vita_glsl_dir = "ux0:/data/advena/glsl"
    project_root = os.path.dirname(BASE_DIR)
...
VITA_GAME_DATA_DIR = "/ux0:/data/advena"
LOCAL_DATA_REFERENCE_DIR = "Advena-1.0.1/assets"
...
def verify_data_assets():
    ...
    el volcado local de referencia (Advena-1.0.1/assets)
    contra lo que realmente está subido en ux0:data/advena/ en
